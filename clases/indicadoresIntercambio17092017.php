...
$tabla = "<table id='tablaResultado'>
			<tr>				
				...
				<th colspan='11' class='colorIndicador_im1'>DIRECCIÓN TÉCNICA ZONAL " . $idZona . "</th>
			</tr>
			<tr>
...
function RevisarOrgMes($zona, $mes, $tipoOrg)
{
	...
	// {
	// 	// SE REVISA SI LA ORGANIZACION ES ANTIGUA
	...
	//  SE MANTIENE ESTA PARTE DEL CODIGO POR POSIBLES CAMBIOS A FUTURO
	/*$sqlOrgMes = "select si.cod_u_organizaciones, si.fecha_registro, si.cod_zona, si.servicio, si.tipo_servicio from im_servicios si inner join u_organizaciones u on (u.cod_u_organizaciones = si.cod_u_organizaciones) where month(si.fecha_registro) = " . $mesInd . " and si.cod_zona = " . $zonaInd . " and u.tipo = '" . $tipoOrg . "' and year(si.fecha_registro) = " . $anioInd . " group by si.cod_u_organizaciones";
	...
	for($i = 0; $i < count($orgReportadasMes); $i = $i + 3)
	{
		...
		//		-Si no existen mas registros ademas del reportado en el mes indicado, esta organizacion es nueva y no debe sumar al indicador
...
function RevisarOrgMesNuevas($zona, $mes)
{
	...
	/*$sqlOrgMes = "select si.cod_u_organizaciones, si.fecha_registro, si.cod_zona, si.servicio, si.tipo_servicio from im_servicios si inner join u_organizaciones u on (u.cod_u_organizaciones = si.cod_u_organizaciones) where month(si.fecha_registro) = " . $mesInd . " and si.cod_zona = " . $zonaInd . " and u.tipo = 'org' and year(si.fecha_registro) = " . $anioInd . " group by si.cod_u_organizaciones";
