...
function NumPersonasCapacitadas($orgReportadas, $anio, $mes, $tipoOrg, $zona)
{
	...
	foreach($orgReportadas as $valor)
	{
		...
		$sqlSociosOrg = "select cedula from socios where cod_u_organizaciones = " . $valor . " group by cedula";
...
function segundo_indicador($mes,$zona)
{
	...
	if($mes != -1)
	{
		$sqlEventosMes .= " and month(e.fecha_registro) = " . $mes;			
	...
	if($mes != 1)
	{
		...
		for($i = 1; $i < $mes; $i++)
		{
			$sqlEventosAnteriores = "select ev.cod_evento, e.fecha_reporte, e.tipo_evento from eventos_u_organizaciones ev inner join eventos e on (e.cod_evento = ev.cod_evento) where e.tipo_evento = 'uep' and ev.anio = ". $anioCurso ." and e.anio = " . $anioCurso ." and month(e.fecha_registro) = " . $i . " and e.zona = " . $zona . " group by ev.cod_evento";
			//echo $sqlEventosAnteriores . "<br>";
			$resEventosAnteriores = query($sqlEventosAnteriores);
			while($fila = mysql_fetch_array($resEventosAnteriores))
			{
